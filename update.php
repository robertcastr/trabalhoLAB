...
if (!empty($_POST)) {
    ...
    if (empty($numero)) {
        $numeroErro = 'Por favor preenche o campo!';
        $validacao = false;
    }
    ...
    if ($validacao) {
        ...
        $sql = "UPDATE pessoa  set nome = ?, endereco = ?, telefone = ?, email = ?, numero = ? WHERE id = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($nome, $endereco, $telefone, $email, $numero, $id));
        Banco::desconectar();
        header("Location: index.php");
    }
} else {
            ...
            <div class="card-body">
                <form class="form-horizontal" action="update.php?id=<?php echo $id ?>" method="post">
                    ...
                    <div class="control-group <?php echo !empty($numeroErro) ? 'error' : ''; ?>">
                        ...
                        <div class="controls">
                            <input name="email" class="form-control" size="40" type="text" placeholder="Numero"
                                   value="<?php echo !empty($email) ? $numero : ''; ?>">
                            <?php if (!empty($numeroErro)): ?>
                                <span class="text-danger"><?php echo $numeroErro; ?></span>
                            <?php endif; ?>
                        </div>
                    </div>
